...
function patternPoints(idx){
  ...
  if(mode===0){
    for(let ring=0;ring<12;ring++)for(let j=0;j<24;j++){const a=j/24*Math.PI*2+ring*.135,r=.36+ring*.22,petal=1+.09*Math.sin(a*6+ring*.42);pts.push([Math.cos(a)*r*petal,Math.sin(a)*r*.70,ring/11,j/24]);}
  }else if(mode===1){
    for(let arm=0;arm<8;arm++)for(let j=0;j<36;j++){const t=j/35,a=arm/8*Math.PI*2+t*Math.PI*3.45,r=.24+t*2.78;pts.push([Math.cos(a)*r,Math.sin(a)*r*.71,t,arm/8]);}
  }else if(mode===2){
    for(let y=0;y<16;y++)for(let x=0;x<18;x++){const xx=(x-8.5)*.33+Math.sin(y*.78)*.07,yy=(y-7.5)*.24+Math.sin(x*.63+y*.25)*.045;pts.push([xx,yy,y/15,x/17]);}
  }else if(mode===3){
    for(let petal=0;petal<16;petal++)for(let j=0;j<18;j++){const t=j/17,a=petal/16*Math.PI*2+Math.sin(t*Math.PI)*.23,r=.30+t*2.55,wave=1+.11*Math.sin(t*Math.PI*3+petal*.8);pts.push([Math.cos(a)*r*wave,Math.sin(a)*r*.72*wave,t,petal/16]);}
  }else{
    for(let fan=0;fan<16;fan++)for(let j=0;j<18;j++){const t=j/17,spread=(fan-7.5)/7.5,fanOpen=.38+.72*t,r=.36+t*2.4;const x=spread*fanOpen*r+.06*Math.sin(fan*.7+t*3.5),y=-1.15+t*2.95+.08*Math.cos(fan*.45+t*2.2);pts.push([x,y*.76,t,fan/16]);}
  }
  return pts;
}
...
function phaseLocalTime(){
  const phase=sequenceState.phase??sequenceState.shot.phase??0;let t=sequenceState.segment;
  for(let i=0;i<sequenceState.index;i++)if(CAMERA_SHOTS[i].phase===phase)t+=CAMERA_SHOTS[i].duration;
  return t;
}
function shotFieldScale(){
  const c=sequenceState.shot.clarity;
  if(c==='HERO_FX')return .72;
  if(c==='COMBAT_CLOSE')return .82;
  if(c==='ITANO')return .90;
  if(c==='COMBAT_WIDE')return 1.04;
  return .94;
}
function cardPositions(local){
  const arr=[],pt=phaseLocalTime(),open=easeOut(smooth((pt-.10)/1.05)),profile=visualProfile();
  for(let i=0;i<CARD_COUNT;i++){
    const anchor=cardAnchors[i],a=Math.atan2(anchor.y,anchor.x),breath=.065*Math.sin(time*.63+i*1.71),spread=(.64+breath)*(profile.cardScale>.9?1:.92);
    const px=bossPos[0]+anchor.x*spread*open+.055*Math.sin(i*2.17+time*.17)*open;
    const py=bossPos[1]+anchor.y*.55*open+.055*Math.cos(i*1.37+time*.22)*open;
    const pz=bossPos[2]+(anchor.layer-.5)*1.95*open+.20*Math.sin(a*2.0+time*.14+i*.19)*open;
...
function targetWorld(s,local){
  const mode=sequenceState.shot.mode,activeT=Math.max(0,phaseLocalTime()-.35),fs=shotFieldScale();
  let rot=0,drift=.015+(.007*Math.sin(time*.78+s.seed)),sx=1,sy=1;
  if(mode===0){rot=.10*Math.sin(activeT*1.4)+.15*activeT;sy=.96;}
  else if(mode===1){rot=.07*Math.sin(activeT*1.2)-.10*activeT;sx=.92;sy=.84;}
  else if(mode===2){rot=.018*Math.sin(activeT*1.8);sx=1.05;sy=.92;}
  else if(mode===3){rot=-.12*Math.sin(activeT*1.25)-.085*activeT;sx=.94;sy=.98;}
  else {rot=.025*Math.sin(activeT*2.0)+.05*activeT;sx=1.08;sy=.88;drift=.020;}
  const cr=Math.cos(rot),sr=Math.sin(rot),wave=Math.sin(time*.78+s.seed);
  const x=((s.x*cr-s.y*sr)*sx+drift*Math.cos(s.seed)+wave*.014*(mode===4?1.5:1.0))*fs;
  const y=((s.x*sr+s.y*cr)*sy+drift*Math.sin(s.seed*.73))*fs;
